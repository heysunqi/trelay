...
- **编辑连接配置**：按下E键，显示编辑对话框，支持修改已有连接的所有配置项（名称、地址、协议、认证方式、分组等），自动处理分组变更
- **新建分组功能**：按下G或g键创建新分组，支持直接在TUI中管理分组
- **交互式表单**：
  - 连接协议支持SSH、RDP、VNC的下拉选择
  - SSH协议支持密码和密钥认证方式
  - 服务器分组支持直接输入，不存在的分组自动创建
  - 完整的输入验证和格式校验
  - 支持描述信息输入并持久化到配置文件
- **界面优化**：主机数量和键盘快捷键提示居中显示在底部
- **下拉列表选中标记**：下拉选择菜单中已选中的选项会显示标记，便于识别当前选择
- **对话框居中显示**：所有弹窗（包括密码输入、新建连接配置、编辑连接配置、错误提示、新建分组）都实现了水平和垂直居中显示
...
| **服务器分组** | 文本 | 可选，主机所属分组 | 直接输入分组名，不存在则自动创建 |
| **描述信息** | 文本 | 可选，主机描述 | 任意文本 |

#### 使用说明
1. **导航字段**：使用 `Tab` 键前进，`Shift+Tab` 键后退
2. **选择字段**：对于下拉菜单（连接协议、认证方式），按 `Space` 键打开菜单，使用 `↑/↓` 键选择，按 `Enter` 键确认选择；分组字段直接输入分组名称
...
**操作步骤**：
- 直接输入分组名称
- 如果输入的分组名称不存在，保存时会自动创建新分组
- 默认为"未分组"

**分组管理**：
- 分组信息会保存到配置文件中
- 也可以通过 `G/g` 快捷键在主界面直接创建新分组
...
#### 界面优化
- **描述列空间分配优化**：终端多余宽度优先分配给描述列（而非名称列），描述信息展示更充分

#### 问题修复
- **文本字段空格输入修复**：修复新建/编辑连接对话框中 Space 键被下拉菜单逻辑拦截，导致描述、名称等文本字段无法输入空格的问题
- **状态列着色位置修复**：修复有描述列时状态列颜色错误地应用到描述列上的问题
- **描述列颜色丢失修复**：修复状态列 ANSI 重置码导致其后的描述列文字失去颜色的问题，改为分段渲染
- **选中行状态可见性修复**：修复选中行（绿底）下 online/connected 状态文字为绿色导致不可见的问题，选中时改用深绿色
...
#### 新增功能
- **编辑连接配置**：按 `E` 键可编辑选中主机的连接配置
  - 支持修改所有配置项（名称、地址、端口、用户名、协议、认证方式、密码、密钥路径、分组、描述等）
  - 自动处理分组变更（从旧分组移除、添加到新分组、名称变更时更新分组引用）
  - 编辑对话框预填充当前主机配置信息
...
- **下拉列表选中标记**：下拉选择菜单中已选中的选项增加标记显示

#### 性能优化
- **状态检查异步加载**：优化机器状态同步加载造成程序卡顿问题，改为异步并发检查

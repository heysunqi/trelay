// Package tui 实现了基于 Bubble Tea 的终端用户界面
//
// TUI 架构说明:
//
// 1. 状态管理: App 结构体包含所有 UI 状态，包括:
//    - 搜索、分组选择、命令模式等交互模式
//    - 对话框状态 (新建/编辑连接、密码输入、错误提示等)
//    - 连接管理 (当前连接、后台会话列表)
//    - 主机列表和分页状态
//
// 2. 消息流程:
//    - tea.WindowSizeMsg: 终端大小变化
//    - tea.KeyMsg: 键盘输入 (主处理逻辑)
//    - 自定义消息: statusCheckMsg, sshConnectResultMsg 等
//
// 3. 渲染层次 (View 方法):
//    - 对话框层: 连接对话框、密码对话框、错误对话框、分组选择等
//    - 列表层: 主机列表或分组选择列表
//    - 输入层: 搜索框或命令输入框
//    - 头部: 分组信息和快捷键提示
//    - 底部: 状态栏
//
// 4. 设计模式:
//    - 对话框: 各自实现独立的 Update/View 方法
//    - SSH 连接: 使用 protocol.Manager 管理后台会话
//
// 注意事项:
//    - Update 方法较大 (~800行)，包含所有键盘事件处理
//    - View 方法较大 (~900行)，按优先级渲染各层
//    - NewConnectionDialog 和 EditConnectionDialog 代码高度相似
